...
{
  // DrawPlayground();
  Shape currentShape = playGround.PrepareInitialShapePosition(shapes[currentShapeIndex], config);
  bool isLanded = false;
  while (!isLanded)
  {
    // wind
    Direction direction = directions[currentDirectionIndex];
    bool preventMovingWithWind =
      currentShape
        .WindPoints(direction)
        .Any(windPoint => 
          playGround.Contains(
    ...
    bool canLand =
      currentShape
        .LandingPoints()
        .Any(landingPoint => 
          playGround.Contains(new Point(landingPoint.X, landingPoint.Y + 1)) || 
